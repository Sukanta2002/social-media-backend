...
const toFollow = asyncHandler(async (req, res) => {
    ...
    const isFollowed = await Follow.findOne({
        $and: [{ follower: req.user._id }, { toFollowing: userToFollow._id }],
    });

    if (isFollowed) {
        return res
            .status(200)
            .json(new ApiResponce(200, isFollowed, "Already Followed"));
    }
    ...
    return res
        .status(200)
        .json(new ApiResponce(200, follow, "Followed Successfully"));
});
...
const unFollow = asyncHandler(async (req, res) => {
    ...
    if (!follow) {
        return res
            .status(200)
            .json(new ApiResponce(200, {}, "User is already unfollowed"));
    }
